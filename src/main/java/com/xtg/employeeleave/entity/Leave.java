package com.xtg.employeeleave.entity;
...
import com.xtg.employeeleave.util.enums.LeaveStatus;
...
@NamedNativeQuery(
        ...
        query =
                "select count(leave_id) as count, lt.type_name AS leaveType, month(lr.from_date) as month, lr.status as status\n" +
                        ...
                        "    where \n" +
                        "    lr.from_date >= DATEADD(YEAR,-1, GETDATE()) OR\n" +
                        "    lr.to_date >= DATEADD(YEAR,-1, GETDATE())\n" +
